# utils codes of reader_module. The codes are based on huggingface pre-trained-bert libraries.
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        if no_masking is True:
            ...
            if no_masking is True:
                if len(nbest) == 1:
                    nbest.insert(0,
                                 _NbestPrediction(text="", start_logit=0.0, end_logit=0.0, no_answer_logit=1.0,
                                                  switch=1, switch_logits=[0.0, 0.0, 0.0, 0.0]))
            else:
                if len(nbest) == 0:
                    nbest.insert(0,
                                 _NbestPrediction(text="", start_logit=0.0, end_logit=0.0, no_answer_logit=1.0,
        ...
        if not nbest:
            nbest.append(
                _NbestPrediction(text="", start_logit=0.0, end_logit=0.0, no_answer_logit=1.0, switch=1,
                                 switch_logits=[0.0, 0.0, 0.0, 0.0]))
