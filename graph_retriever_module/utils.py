...
class GraphRetrieverConfig:
    ...
    def __init__(self,
        ...
        self.expand_links_ = expand_links
        if self.no_links:
            self.expand_links_ = False
    ...
    def __str__(self):
        configStr = '\n\n#########################################\n' \
            ...
            self.pruning_by_links) + '\n' \
                                     '- Exapnd links (for NQ): ' + str(self.expand_links_) + '\n' \
                                                                                             '- Eval chunk: ' + str(
            self.eval_chunk) + '\n' \
...
def expand_links_(context, all_linked_paras_dic, all_paras):
...
class DataProcessor:
    ...
    * set_type: "train" or "dev"
    ...
    @staticmethod
    def _create_examples(file_name, graph_retriever_config, set_type):
        ...
        for (_, data) in enumerate(tqdm(jsn, desc='Example')):
            ...
            *** Training time: to take a balance between TF-IDF-based and link-based negative examples ***
            '''
            ...
            if set_type == 'dev' and graph_retriever_config.expand_links_:
                expand_links_(context, all_linked_paras_dic, all_paras)
