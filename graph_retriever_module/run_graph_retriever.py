...
def main():
    ...
    parser.add_argument('--task',
                        ...
                        default=None,
                        required=True,
                        help="Task code in {hotpot_open, hotpot_distractor}")

    # Other parameters 可选参数
    ...
    """
     什么是Warmup?
        它在训练开始的时候先选择使用一个较小的学习率，训练了一些epoches或者steps(比如4个epoches,10000steps),再修改为预先设置的学习来进行训练。
     为什么使用Warmup?
     ...
     比如warmup_proportion=0.1，总步数=100，那么warmup步数就为10。在1到10步中，学习率会比10步之后低，10步之后学习率恢复正常。
    """
    ...
    # store_true 当这一选项存在时，为 args.no_cuda 赋值为 True。没有指定时则隐含地赋值为 False。
    ...
    parser.add_argument("--expand_links_",
                        action='store_true',
    ...
    logger.info("torch.cuda.device_count(): %d", n_gpu)
    ...
    graph_retriever_config = GraphRetrieverConfig(example_limit=args.example_limit,
                                                  ...
                                                  expand_links=args.expand_links_,
    ...
    if do_train:
        ...
        for _ in range(int(args.num_train_epochs)):
            logger.info('Epoch ' + str(epc + 1) + '/' + str(int(args.num_train_epochs)))
            ...
            while train_start_index < TOTAL_NUM:
                ...
                for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                    # 字段顺序 all_input_ids, all_input_masks, all_segment_ids, all_output_masks,all_num_paragraphs, all_num_steps
                    input_masks = batch[1]
                    # 句子最大长度
