...
# from graph_retriever_module.utils import expand_links_
# from graph_retriever_module.utils import tokenize_paragraph
# from graph_retriever_module.utils import tokenize_question
...
try:
    ...
    from utils import expand_links_
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, output_mask, target, max_num_steps):
        ...
        for i in range(max_num_steps):
            if i == 0:
                ...
                temp_input = paragraphs[:, i - 1:i, :]  # (B, 1, D)
                state = torch.cat((state, temp_input), dim=2)  # (B, 1, 2*D)
    ...
    def beam_search(self, input_ids, token_type_ids, attention_mask, examples, tokenizer, retriever, split_chunk):
        ...
        for i in range(B):
            ...
            for j in range(self.graph_retriever_config.max_select_num):
                if j > 0:
                    temp_input = [p[0][-1] for p in pred_]
                    temp_input = torch.LongTensor(temp_input).to(ps.device)
                    temp_input = ps[temp_input].unsqueeze(1)  # (beam, 1, D)
                    state_ = torch.cat((state_, temp_input), dim=2)  # (beam, 1, 2*D)
                ...
                if j > 0:
                    ...
                    if len(new_titles) > 0:
                        ...
                        if retriever is not None and self.graph_retriever_config.expand_links_:
                            expand_links_(examples[i].all_paras, examples[i].all_linked_paras_dic,
                                          examples[i].all_paras)

                output = torch.bmm(state_, ps.unsqueeze(0).expand(beam, ps.size(0), ps.size(1)).transpose(1,
                                                                                                          2))  # (beam, 1, N+1)
                output = output + self.bias
