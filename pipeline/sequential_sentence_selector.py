...
from sequential_sentence_selector_module.run_sequential_sentence_selector import DataProcessor
from sequential_sentence_selector_module.run_sequential_sentence_selector import convert_examples_to_features
...
class SequentialSentenceSelector:
    def __init__(self,
        ...
        self.tokenizer = BertTokenizer.from_pretrained(args.bert_model_sequential_sentence_selector,
                                                       do_lower_case=args.do_lower_case)
        model_state_dict = torch.load(args.sequential_sentence_selector_path)
        self.model = BertForSequentialSentenceSelector.from_pretrained(args.bert_model_sequential_sentence_selector,
    ...
    def predict(self,
        ...
        eval_features = convert_examples_to_features(
            eval_examples, args.max_seq_length_sequential_sentence_selector, args.max_sent_num, args.max_sf_num,
            self.tokenizer)
        ...
        for input_ids, input_masks, segment_ids, output_masks, num_sents, num_sfs, ex_indices in tqdm(eval_dataloader,
                                                                                                      desc="Evaluating"):
            ...
            with torch.no_grad():
                pred, prob, topk_pred, topk_prob = self.model.beam_search(input_ids, segment_ids, input_masks,
                                                                          output_masks,
                                                                          max_num_steps=args.max_sf_num + 1,
                                                                          examples=examples,
                                                                          beam=args.beam_sequential_sentence_selector)
            ...
            for i in range(len(pred)):
                ...
                for p in pred[i]:
                    ...
                    for j in range(len(e.titles)):
                        if offset <= p < offset + len(e.context[e.titles[j]]):
                ...
                output = {'q_id': e.guid, 'supporting facts': sfs}
