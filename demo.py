...
class MyApp(App):
    ...
    def on_button_pressed_question(self, widget):
        ...
        txt_retrieval_path_dict = graph_retriever_output[0]['context']
        txt_retrieval_path = '\n'
        txt_retrieval_path_list = list(txt_retrieval_path_dict.values())
        for temp_context_index in range(len(txt_retrieval_path_list)):
            if temp_context_index != len(txt_retrieval_path_list) - 1:
                txt_retrieval_path = txt_retrieval_path + txt_retrieval_path_list[temp_context_index] + "\n----->\n"
            else:
                txt_retrieval_path = txt_retrieval_path + txt_retrieval_path_list[temp_context_index]
        print(txt_retrieval_path)
        self.txt_retrieval.set_text("推理路径: " + txt_retrieval_path)
        ...
        if len(supporting_facts) > 0:
            ...
            txt_facts_dict = supporting_facts[0]['supporting facts']
            # print(list(txt_facts_dict.values()))
            txt_facts_list = [i[0][1] for i in list(txt_facts_dict.values())]

            txt_facts_path = '\n'
            for index in range(len(txt_facts_list)):
                if index != len(txt_facts_list) - 1:
                    txt_facts_path = txt_facts_path + txt_facts_list[index] + "\n&\n"
                else:
                    txt_facts_path = txt_facts_path + txt_facts_list[index]
            self.txt_facts.set_text('事实依据:' + str(txt_facts_path))
